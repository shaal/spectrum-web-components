...
export class Menu extends SpectrumElement {
    ...
    private onItemAddedSelectable(
        ...
        const roleIsPresentation = this.getAttribute('role') === 'presentation';
        let managed = !!this.selects && !roleIsPresentation;
        if (event.detail.owned) {
            managed = false;
        } else if (this.selects) {
            event.detail.owned = !roleIsPresentation;
            event.detail.focusRoot = this;
